...
const MATCHES_DATA = [
  ...
  { id:41, group:"G", home:"🇪🇬 Egypt",          away:"🇮🇷 IR Iran",         utc:Date.UTC(2026,5,27,3,0),   round:"Round 3" }, // 11pm ET Jun 26
  { id:42, group:"G", home:"🇳🇿 New Zealand",   away:"🇧🇪 Belgium",         utc:Date.UTC(2026,5,27,3,0),   round:"Round 3" }, // 11pm ET Jun 26
  // GROUP H
  ...
  { id:47, group:"H", home:"🇨🇻 Cabo Verde",     away:"🇸🇦 Saudi Arabia",   utc:Date.UTC(2026,5,27,0,0),   round:"Round 3" }, // 8pm ET Jun 26
  { id:48, group:"H", home:"🇺🇾 Uruguay",        away:"🇪🇸 Spain",           utc:Date.UTC(2026,5,27,0,0),   round:"Round 3" }, // 8pm ET Jun 26
  // GROUP I
  ...
  { id:53, group:"I", home:"🇳🇴 Norway",         away:"🇫🇷 France",          utc:Date.UTC(2026,5,26,19,0),  round:"Round 3" }, // 3pm ET
  { id:54, group:"I", home:"🇸🇳 Senegal",        away:"🇮🇶 Iraq",            utc:Date.UTC(2026,5,26,19,0),  round:"Round 3" }, // 3pm ET
  // GROUP J
  ...
  { id:59, group:"J", home:"🇩🇿 Algeria",        away:"🇦🇹 Austria",         utc:Date.UTC(2026,5,28,2,0),   round:"Round 3" }, // 10pm ET Jun 27
  { id:60, group:"J", home:"🇯🇴 Jordan",         away:"🇦🇷 Argentina",       utc:Date.UTC(2026,5,28,2,0),   round:"Round 3" }, // 10pm ET Jun 27
  // GROUP K
  ...
  { id:65, group:"K", home:"🇨🇴 Colombia",       away:"🇵🇹 Portugal",        utc:Date.UTC(2026,5,27,23,30), round:"Round 3" }, // 7:30pm ET
  { id:66, group:"K", home:"🇨🇩 Congo DR",       away:"🇺🇿 Uzbekistan",      utc:Date.UTC(2026,5,27,23,30), round:"Round 3" }, // 7:30pm ET
  // GROUP L
  ...
  { id:71, group:"L", home:"🇵🇦 Panama",         away:"🏴󠁧󠁢󠁥󠁮󠁧󠁿 England",       utc:Date.UTC(2026,5,27,21,0),  round:"Round 3" }, // 5pm ET
  { id:72, group:"L", home:"🇭🇷 Croatia",        away:"🇬🇭 Ghana",           utc:Date.UTC(2026,5,27,21,0),  round:"Round 3" }, // 5pm ET
...
const CONFETTI_COLORS = ["#f1c40f","#e67e22","#e74c3c","#3498db","#9b59b6","#1abc9c"];
...
const CongratsOverlay=({rank,championName,onDismiss})=>{
  ...
  return(
    <div style={{position:"fixed",inset:0,zIndex:500,background:"rgba(10,22,40,0.97)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:24,textAlign:"center"}}>
      <div style={{fontSize:15,fontWeight:800,letterSpacing:3,textTransform:"uppercase",color:"#f1c40f",marginBottom:14}}>
        {isChamp?"🏆 Congratulations Champion":"🤝 Good Fight"}
      </div>
      <div style={{fontSize:38,fontWeight:900,color:"#fff",lineHeight:1.15,marginBottom:isChamp?24:14}}>
        You Finished {isChamp?"1st":ordinal(rank)}!
      </div>
      {!isChamp&&(
        <div style={{fontSize:15,color:"#7f8c9a",fontWeight:600,marginBottom:26,maxWidth:320}}>
          {championName} smoked your ass 😂😂
        </div>
      )}
      <button
        style={{background:"linear-gradient(135deg,#f1c40f,#e67e22)",border:"none",borderRadius:10,padding:"13px 30px",color:"#0a1628",fontWeight:800,fontSize:14,cursor:"pointer",fontFamily:"inherit",textTransform:"uppercase"}}
        onClick={onDismiss}
      >
        View Points
      </button>
...
export default function App() {
    ...
    return(
      ...
      <div style={s.hdr}>
        ...
        <div style={{display:"flex",gap:5,alignItems:"center"}}>
          <LiveDot/>
          <button style={s.nb} onClick={()=>{setShowCongrats(true);setView("leaderboard");}}>🏅 {getBoard().length}</button>
  ...
  if(view==="leaderboard"){
    ...
    const allTimeRanked=getBoard();
    const myEntry=allTimeRanked.find(p=>p.isMe);
    const champion=allTimeRanked.find(p=>p.rank===1);
    const finalPlayed=results[104]?.home!==undefined;
    const showOverlay=finalPlayed&&showCongrats&&!!myEntry&&!!champion;
      ...
      <div style={{background:"#0a1628",minHeight:"100vh",fontFamily:"'Barlow Condensed',sans-serif",color:"#fff",position:"relative",overflow:"hidden"}}>
        ...
        {showOverlay&&(
          <CongratsOverlay
            ...
            onDismiss={()=>{setShowCongrats(false);setLbRound("all");}}
          />
        )}
