...
const ScoreInput=({val,locked,onChange,onDone})=>{
  const num=val===""?null:parseInt(val);
  const dec=()=>{
    if(locked)return;
    const n=num===null?0:Math.max(0,num-1);
    onChange(String(n));
    setTimeout(onDone,100);
  };
  const inc=()=>{
    if(locked)return;
    const n=num===null?0:Math.min(20,num+1);
    onChange(String(n));
    setTimeout(onDone,100);
  };
  return(
    <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:2}}>
      {!locked&&<button onMouseDown={e=>{e.preventDefault();inc();}} style={{width:36,height:24,background:"rgba(241,196,15,0.15)",border:"1px solid rgba(241,196,15,0.3)",borderRadius:5,color:"#f1c40f",fontSize:16,fontWeight:900,cursor:"pointer",lineHeight:1,touchAction:"manipulation"}}>+</button>}
      <div style={{width:36,height:36,display:"flex",alignItems:"center",justifyContent:"center",background:locked?"rgba(255,255,255,0.03)":"rgba(255,255,255,0.08)",border:locked?"1px solid rgba(255,255,255,0.05)":"2px solid rgba(241,196,15,0.4)",borderRadius:7,fontSize:20,fontWeight:900,color:locked?"#555":"#f1c40f",minWidth:36}}>
        {num===null?"–":num}
      </div>
      {!locked&&<button onMouseDown={e=>{e.preventDefault();dec();}} style={{width:36,height:24,background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:5,color:"#7f8c9a",fontSize:16,fontWeight:900,cursor:"pointer",lineHeight:1,touchAction:"manipulation"}}>–</button>}
    </div>
  );
};
...
export default function App() {
  ...
  const saveJoker=async(matchId)=>{
    setJoker(matchId);
    if(playerId) await update(ref(db,`players/${playerId}`),{jokerUsed:matchId});
    toast2("🃏 Joker played! 2× points!");
  };

  const removeJoker=async()=>{
    setJoker(null);
    if(playerId) await update(ref(db,`players/${playerId}`),{jokerUsed:null});
    toast2("🃏 Joker removed — you can play it on another match");
  };

  const myPts=MATCHES_DATA.reduce((acc,m)=>{
    const a=results[m.id];if(!a)return acc;
    const p=preds[m.id];if(!p)return acc;
    let pt=calcPoints(p,a);if(joker==m.id)pt*=2;
    return acc+pt;
  },0);

  const getBoard=()=>Object.entries(allPlayers).map(([id,player])=>{
    let pts=0;
  ...
  const MatchCard=({m,isToday})=>{
    ...
    const lk=locked(m);
    const cd=!lk?formatCountdown(m.utc-tick):null;
    return(
      <div style={{...s.mc,...(isJ?s.jc:{}),...(lk?s.lc:{})}}>
        ...
        {!lk&&<div style={s.ac}>{!joker&&<button style={s.jb} onClick={()=>saveJoker(m.id)}>🃏 Play Joker</button>}{isJ&&<div style={{display:'flex',alignItems:'center',gap:8}}><span style={s.ja}>🃏 JOKER ACTIVE — 2×</span><button style={{background:'rgba(231,76,60,0.12)',border:'1px solid rgba(231,76,60,0.4)',borderRadius:5,padding:'3px 9px',color:'#e74c3c',fontSize:10,fontWeight:700,cursor:'pointer',fontFamily:'inherit'}} onClick={removeJoker}>✕ Remove</button></div>}</div>}
      </div>
    );
  };
  ...
  if(view==="leaderboard"){
    ...
    return(
      ...
      <div style={s.lbwrap}>
        <div style={s.lbsub}>{board.length} player{board.length!==1?"s":""} · {Object.keys(results).length} result{Object.keys(results).length!==1?"s":""} in · 🔴 Live</div>
        {board.length===0&&(<div style={{textAlign:"center",padding:"40px",color:"#7f8c9a"}}><div style={{fontSize:36,marginBottom:10}}>👥</div><div style={{fontSize:14,color:"#fff"}}>No players yet</div><div style={{fontSize:12,marginTop:5}}>Share the link with your friends!</div></div>)}
        {board.map(p=>(
